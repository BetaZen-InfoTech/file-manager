...
import { testSource, discoverSource, runMigration } from '@/lib/migration';
...
function redact(j: any) {
  ...
  if (o.source) {
    o.source = { ...o.source, accessKeyEnc: undefined, secretKeyEnc: undefined };
  }
  return o;
}
...
export async function POST(req: NextRequest) {
  ...
  const { action, source, targetVendorId, targetBucketName } = parsed.data;

  if (action === 'test') return jsonOk(await testSource(source));
  if (action === 'discover') return jsonOk(await discoverSource(source));

  // start
  if (!targetVendorId || !targetBucketName) {
    return badRequest('targetVendorId and targetBucketName are required to start');
  }
  await dbConnect();
  const vendor = await Vendor.findById(targetVendorId).lean();
  if (!vendor) return badRequest('target vendor not found');

  // Verify the source is reachable before creating a job.
  ...
  await audit(p, req, { action: 'migration.start', resourceType: 'migration', resourceId: String(job._id) });

  // Run in the background (long-running PM2 process keeps it alive).
