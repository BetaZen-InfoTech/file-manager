...
if [[ ! -f "$ENV_FILE" || "${DO_RESET:-0}" == "1" ]]; then
  ...
  # Secure cookies are only stored by browsers over HTTPS. Start optimistic
  # (HTTPS expected) unless SSL is being skipped; the SSL step below re-syncs
  # this to the real TLS state so HTTP-only installs can still log in.
  if [[ "$SKIP_SSL" -eq 1 ]]; then COOKIE_SECURE_VALUE=false; else COOKIE_SECURE_VALUE=true; fi
...
# ============================================================================
# 13b. Sync COOKIE_SECURE to the actual TLS state
# ============================================================================
# Browsers only store a `Secure` session cookie over HTTPS. If HTTPS is now live
# keep Secure cookies (the safe default); otherwise (--skip-ssl or certbot
# failed) switch to a non-Secure cookie so login works over http://<domain-or-ip>
...
if grep -q '^COOKIE_SECURE=' "$ENV_FILE"; then
  sed -i "s/^COOKIE_SECURE=.*/COOKIE_SECURE=$DESIRED_COOKIE_SECURE/" "$ENV_FILE"
else
...
if [[ "$DESIRED_COOKIE_SECURE" == "true" ]]; then
  ...
  warn "COOKIE_SECURE=false — no HTTPS yet, so the session cookie is sent over plain HTTP."
  warn "Login works over http://, but it's sniffable. After enabling SSL (sudo bcdnp /"
  warn "certbot), set COOKIE_SECURE=true in $ENV_FILE and: pm2 reload filemanager --update-env"
fi
